// Pull representative colours out of the current cover art for the player's
// tint. The controller serves /cover/* with a wide-open CORS header, so a
// crossOrigin image can be drawn to a canvas and read back without tainting; any
// failure (taint, decode error, no support) resolves to nulls and skips the tint.

import { useEffect, useState } from 'react';

export interface CoverColors {
  // Saturation-weighted dominant colour. `rgb(r, g, b)` string, or null if
  // extraction hasn't run or failed.
  vibrant: string | null;
  // Flat average, a calmer companion for the gradient's tail.
  average: string | null;
}

const EMPTY: CoverColors = { vibrant: null, average: null };

// Downscale target: enough to find dominant hues, and keeps the per-track
// getImageData walk to ~576 pixels.
const SAMPLE = 24;
...
// Cheap saturation proxy (max-min over max), 0..1. Greys score ~0 so they can't
// win the dominant-colour vote against a smaller but vivid region.
    ...
    img.onload = () => {
      ...
      try {
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          // Near-black and blown-out near-white both make muddy washes, so
          // down-weight them and let mid-tone vivid colours dominate.
